...
public partial class CreateDebtViewModel(ICollectorDebtService debtService) : ObservableObject
{
    ...
    private string _editDebtId = string.Empty;

    public Func<Task>? OnDebtCreated { get; set; }

    public bool IsEditMode => !string.IsNullOrWhiteSpace(_editDebtId);
    public string PageTitle => IsEditMode ? "Editar deuda" : "Nueva deuda";
    public string SaveButtonText => IsEditMode ? "Guardar cambios" : "Crear deuda";
    ...
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(IsNotLoading), nameof(CanCreate))]
    private bool _isLoading;
    public bool IsNotLoading => !IsLoading;
    public bool CanCreate => !IsLoading && !IsEditBlocked;
    ...
    async Task CreateAsync()
    {
        ...
        {
            if (IsEditMode)
                await debtService.UpdateAsync(ClientId, _editDebtId, request);
            else
        ...
        {
            EditBlocked = true;
            GeneralError = "Esta deuda ya tiene pagos registrados y no puede editarse.";
        }
        ...
        catch (Exception)
        {
            GeneralError = IsEditMode
                ...
                : "No pudimos crear la deuda. Intenta nuevamente.";
        }
        finally
        {
            IsLoading = false;
    ...
    public void LoadForEdit(CollectorDebtDetailResponse debt, string clientName)
    {
        ...
        try
        {
            ...
            ClientInitials = BuildInitialsFrom(clientName);
            ClientAvatarColor = AvatarColorFrom(clientName);

            Periodicidad = debt.Periodicity?.Trim().ToLowerInvariant() switch
            {
                "unica" or "única" or "once" => Periodicidad.Unica,
                "semanal" or "weekly"         => Periodicidad.Semanal,
                "mensual" or "monthly"        => Periodicidad.Mensual,
                _                             => Periodicidad.Quincenal
            };

            CalculationMode = debt.CalculationMode?.Trim().ToLowerInvariant() switch
            {
                "byduedate" or "by_due_date" => DebtCalculationMode.ByDueDate,
                _                            => DebtCalculationMode.ByInstallmentAmount
            };

            Description   = debt.Description;
            Amount        = FormatEditDecimal(debt.PrincipalAmount);
            PaymentAmount = CalculationMode == DebtCalculationMode.ByInstallmentAmount
                ? FormatEditDecimal(debt.ScheduledInstallmentAmount)
                : string.Empty;
            InterestRate  = debt.NormalInterestRate > 0 ? FormatEditDecimal(debt.NormalInterestRate) : string.Empty;
            MoratoryRate  = debt.MoratoryRate > 0 ? FormatEditDecimal(debt.MoratoryRate) : string.Empty;
            Notes         = debt.Notes ?? string.Empty;

            StartDate        = debt.StartDate.Date;
            FirstPaymentDate = debt.FirstPaymentDate.Date;
        ...
        OnPropertyChanged(nameof(IsEditBlocked));
    ...
    private static string FormatEditDecimal(decimal value)
        => value.ToString("0.##", CultureInfo.InvariantCulture);

    private static string BuildInitialsFrom(string name)
    {
    ...
    private static Color AvatarColorFrom(string name)
    {
