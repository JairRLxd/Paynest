...
public partial class RegisterPaymentViewModel(
    PreviewPaymentUseCase       previewPayment,
    RegisterPaymentUseCase      registerPayment,
    DownloadTicketUseCase       downloadTicket,
    CollectorPaymentSettings    paymentSettings,
    CollectorDataRefreshService refreshService) : ObservableObject
{
    ...
    public void Load(RegisterPaymentSnapshot snapshot)
    {
        ...
        _installmentId = snapshot.InstallmentId;
    ...
    private async Task LoadInitialPreviewAsync()
    {
        ...
        try
        {
            ...
            var preview = await previewPayment.ExecuteAsync(_clientId, req);
    ...
    async Task ConfirmPaymentAsync()
    {
        ...
        try
        {
            ...
            if (response.TicketUrl is not null)
            {
                var wantTicket = await ShowConfirmAsync(
                    "Pago registrado",
                    "El pago fue registrado correctamente. ¿Deseas descargar el ticket?");
                if (wantTicket)
                    await TryDownloadTicketAsync(response.PaymentId, response.TicketUrl);
            }
            else
            {
    ...
    private async Task TryDownloadTicketAsync(string paymentId, string ticketUrl)
    {
        try
        {
            var pdfBytes = await downloadTicket.ExecuteAsync(ticketUrl);
            var path = Path.Combine(FileSystem.CacheDirectory, $"ticket_{paymentId}.pdf");
        ...
        {
            await ShowAlertAsync("No se pudo descargar el ticket", "Puedes solicitarlo más tarde desde el historial de pagos.");
        }
    }

    [RelayCommand]
    async Task GenerateTicketAsync()
        => await ShowAlertAsync("Ticket", "El ticket se genera automáticamente al confirmar el pago.");
